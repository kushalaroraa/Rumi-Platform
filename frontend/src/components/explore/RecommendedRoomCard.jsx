...
export const RecommendedRoomCard = ({
  ...
  return <div className="w-[190px] flex-shrink-0 border border-gray-100 rounded-2xl bg-white overflow-hidden shadow-sm">
      <div className="relative h-24 bg-slate-100">
        <img src={cover} alt={room?.propertyType || 'Room'} className="w-full h-full object-cover" />
        <div className="absolute top-2 left-2 bg-emerald-500 text-white px-2.5 py-1 rounded-full text-[10px] font-semibold shadow">
          {score}% Match
        </div>
      </div>
      ...
      <div className="p-2.5">
        ...
        <div className="mt-1.5 flex items-baseline gap-1">
          <span className="text-base font-bold text-gray-900">₹{room?.monthlyRent ?? 0}</span>
        ...
        <div className="mt-2.5">
          <button type="button" onClick={() => onViewDetails?.(room)} className="w-full px-3 py-1.5 bg-blue-600 text-white rounded-xl text-xs font-medium hover:bg-blue-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed" disabled={!onViewDetails}>
            See room details
          </button>
        </div>
      </div>
