...
export const OTPScreen = ({
  ...
  return <AuthLayout title={title} subtitle={subtitle} onBack={onBack}>
      <div className="mb-8 p-3 bg-blue-50/50 rounded-lg flex items-center justify-between border border-blue-100">
        ...
        <button type="button" onClick={onBack} className="text-xs text-[#4E668A] font-bold hover:underline flex items-center gap-1">
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex justify-between gap-2 mb-8">
          {otp.map((digit, index) => <input key={index} ref={el => inputRefs.current[index] = el} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={e => handleChange(index, e.target.value)} onKeyDown={e => handleKeyDown(index, e)} onPaste={handlePaste} className="w-12 h-14 md:w-14 md:h-16 text-center text-2xl font-bold bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-[#081A35] focus:ring-4 focus:ring-[#081A35]/10 transition-all outline-none text-slate-900" />)}
        </div>
        ...
        <div className="mt-6 text-center">
          {timer > 0 ? <p className="text-slate-400 text-sm">
              Resend code in <span className="font-semibold text-slate-600">00:{timer.toString().padStart(2, '0')}</span>
            </p> : <button type="button" onClick={() => setTimer(30)} className="text-[#4E668A] font-semibold text-sm hover:underline flex items-center justify-center gap-2 mx-auto">
